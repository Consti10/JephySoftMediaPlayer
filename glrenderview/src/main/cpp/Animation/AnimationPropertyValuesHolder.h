...
namespace evo {
    template<typename T>
    class AnimationPropertyValuesHolder {

    public:
        /**
         * hold start and end values, use evaluator to evluate
         * @param m_start_val must be a pointer type e.g. (float *)
         */
        AnimationPropertyValuesHolder(T m_start_val, T m_stop_val, int m_length,
        ...
        virtual ~AnimationPropertyValuesHolder() {
            if (m_this_frame_val != nullptr) {
                free(m_this_frame_val);
            }

            if (m_start_val != nullptr) {
                delete m_start_val;
            }

            if (m_stop_val != nullptr) {
                delete m_stop_val;
            }
        ...
        /**
         * @param setter vlaue setter to set result value back
         */
        void SetValueSetter(ValueSetter *setter) {
            AnimationPropertyValuesHolder::m_value_setter = setter;
        }

        T CalculateValue(float fraction) {
            m_evaluator->FloatEvaluate(fraction, m_start_val, m_stop_val, &m_this_frame_val);
            AnimationPropertyValuesHolder::AnimateValue();
